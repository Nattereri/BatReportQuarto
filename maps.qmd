...
The R package `osmdata` downloads feature data from OpenStreetMap (OSM) [^1]. A list of the available features can be found in the [OSM wiki](https://wiki.openstreetmap.org/wiki/Map_features). @fig-devonrivers shows the _river_ feature abstracted from`osmdata` and displayed on the outline map of Devon from @fig-marytavylocDevon.  @fig-watercourses shows the water features (river, ditch, drain, and stream) for West Devon together with the location of the example data in the `iBats` package.  

```{r}
#| warning: false
#| message: false
#| label: fig-devonrivers
#| fig-cap: "Rivers of Devon"
...
#| warning: false
#| message: false
#| label: fig-watercourses
#| fig-cap: "Location of iBats Example Data and Watercourses of West Devon"
...
#| fig-cap: "Location of Mary Tavy on a World Map (Gall Peters Projection)"
