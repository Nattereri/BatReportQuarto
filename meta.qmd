...
The code below takes the `static_data` set from the *iBats* package and adds the *Night* column, @tbl-night shows a random selection of 5 rows from the `static_data` set.
...
The function `iBats::date_time_info()` adds other date and time information: month, year and time see @tbl-other (note: not all columns are shown). @tbl-other shows a random selection of 5 rows from the `static_data` set.
