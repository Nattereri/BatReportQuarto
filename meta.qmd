...
_Meta Data_[^11] is added as a column or columns to the _tidy_ data.  Even with a minimum tidy data set (i.e. species, datetime and geolocation) many columns of useful meta data can be added to enhance the analysis and reporting; described below is simple meta data easily added to tidy data.

[^11]: Meta data can also be a one line description (e.g. a site telephone number); here we follow the principles of _tidy_ data where all the information is contained in a single row of the data file.
...
The function `iBats::date_time_info()` adds the date and time information: month, year and time see @tbl-other. (Note: the JustTime column is plus 12 hours this aids plotting continuously on one axis as shown in this [graph](https://bat-survey-reporting.netlify.app/visualisation.html#fig-timebat)).  Only a random selection of 5 rows from the `statics` set is shown in @tbl-other.
...
@tbl-suntimes shows a selection of 5 rows from the `statics` data with the `DateTime` column and the `Night` column already added[^10], the function `iBats::sun_night_metrics()` calculates the columns listed above, the following columns are shown: `sunset`, `post_set_min`, and, `post_set_hr_int`.

[^10]: with `iBats::date_time_info()`
...
statics$Common <-  unname(BatCommonUK[statics$Species])
...
{{< include ../Common/bats/batlookuplist.qmd >}}
...
{{< include ../Common/bats/EPSG.qmd >}}


{{< include ../Common/bats/BNG.qmd >}}
...
{{< include ../Common/bats/TSandHibernation.qmd >}}
