...
agg_data <- statics_plus %>%
  ...
  mutate(DateTime15 = lubridate::round_date(DateTime, "15 minutes"),
         DateTime30 = lubridate::round_date(DateTime, "30 minutes"))
...
ggplot(Count30, aes(Passes, fill = Month )) +
  ...
  scale_fill_nejm() +
  labs(x = "Passes (Number / 15 minutes)") +
  theme_bw() +
...
Without assuming the data to have normal distribution, decide at .01 significance level (p < 0.01) if the observations of Common pipistrelle bats (every 15 minutes) for September and October have identical data distribution.
...
The null hypothesis Ho: is that the common pipstrelle observation data for September and October are identical populations. The alternate hypothesis H1: is that the common pipstrelle observation data of September and October are not identical populations.


To test the hypothesis, we apply the wilcox.test function to compare the independent samples. As the p-value turns out to be `r wilcox.test(Passes ~ Month, data=Count30)$p.value`, and is not less than the .01 significance level, we do not reject the null hypothesis. At .01 significance level (p < 0.01), we conclude that the common pipstrelle observation data for September and October are identical populations.
