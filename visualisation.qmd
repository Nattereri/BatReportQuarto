...
ggplot(graph_data, aes(y = reorder(Species, n), x = n, fill = Species)) +
  ...
  geom_label_repel(data = graph_data, aes(label = label),
                   nudge_y = -0.25,
...
ggplot(graph_data, aes(area = n, fill = Species, label = Species, subgroup = Description)) +
  ...
  geom_treemap_subgroup_text(place = "centre", grow = T, alpha = 0.9, colour =
                             "grey20",  min.size = 0) +
  geom_treemap_text(colour = "grey90", place = "topleft", fontface = "italic", 
...
### Observed Emergence Time and Roost Potential 

```{r}
#| label: fig-emergence1
#| fig-cap: "Observed Emergence Time and Roost Potential"
#| warning: false
#| fig.height: 8
...
Caption <- "First 90 minutes after sunset. \nWhite bars are emergence times adapted from (Andrews and Pearson, 2022)"
