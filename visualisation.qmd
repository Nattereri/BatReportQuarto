...
Good visualisation tells a story, removing the noise from the data and illuminating the useful information. We visualise to present information to a lay audience; it also helps interpretation and discussion within the survey team.  Geospatial visualisation is shown on the [Maps](<https://bat-survey-reporting.netlify.app/maps.html>) page. 

Diagrams, charts, figures are ubiquitous; do we still notice them? When a diagram is good it can have great influence, as shown in _100 Diagrams That Changed the World_ [@Scottchristianson2014100:ee53]. Visualisation is the best way of communicating quantitative data and there are many modern examples on how to this; from the ground breaking work of Edward Tufte [@Edwardrtufte1983Visual:1a3f] [@Edwardrtufte2020Seeing:4ec6] through to Alan Smith, at the Financial Times, with _How Charts Work_ [@Alansmith2022Charts:c9ad]. The Royal Statistical Society has published [Best Practices for Data Visualisation](https://royal-statistical-society.github.io/datavisguide/) providing insights, advice, and examples (with code) to make data outputs more readable, accessible, and impactful.  Other examples of data visualisation are: _Information is Beautiful_ [@Davidmccandless2012Information:84d3]; _Data Points - Visualization that means something_ [@yau_data_2013]; and, _Visualize This - The Flowing Data Guide to Design, Visualization, and Statistics_ [@yau_visualize_2011]. 


## Bat Colours  

Colour can enhance visualisation, the choice and combination of colours is important to the communication [@Fabiocrameri2020Misuse:917a]. A consistent colour applied to individual bat species and a group of colours accessible to people with a visual impairment, helps make the visualisations _readable_. Unfortunately, accessibility is challenging with more than 8 or 10 colours; if this is the case consider separating the visualisation into different components, creating more than one graph or just highlighting the key species with one colour, with the remaining species shaded as grey.

@tbl-batcolours lists the set of colours used in the `iBats::bat_colours_default` function and are used for most of the visualisations on this page. The `iBats::bat_colours()` function allows any colour to be associated with any bat name (including common names); @sec-usercolours gives an example chart with common bat names.  The colour palett resource provided by [coolors](https://coolors.co/) lends the ability to create and export a suite of colours; that could be assigned to individual species of bat.
...
@fig-andrews illustrates emergence times for UK bats based on the work of [@HenryandrewsReview:59d3].  
...
Bar charts (@fig-plots-1) and pie charts (@fig-plots-2) are part of the standard repertoire for reporting bat surveys. These _flat_ graphs can be hard to interpret when there are a large number of variables to display, as in this case with the number of different species in @fig-plots; and/or a high value, such as the _Pipistrellus pipistrellus_ species in @fig-plots, that can _hide_ other values.  These charts are more effective when they are interactive; example [pie chart](https://bat-survey-reporting.netlify.app/dynamic.html#pie-chart) and [bar charts](https://bat-survey-reporting.netlify.app/dynamic.html#bar-chart) are demonstrated on the [Interactive Reports](https://bat-survey-reporting.netlify.app/dynamic.html) page.
