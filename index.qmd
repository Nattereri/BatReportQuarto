...
The term _Data Science_ is used, as this covers data collection, management, processing, analysis, visualisation, interpretation and reporting. Note: statisticians would state this process is what they have always done [@Daviddonoho201750:9bd5].

The data science is applied through literate programming [@knuth_literate_1984] outlined in @fig-workflowred. This enables efficient reporting of bat data[^8] from a simple table, such as a count of bats, to the output and interpretation of machine learning in a fully formatted report; plus everything in-between, all accomplished through open source R[^9] [@R-base] and RStudio [@RStudio22]. 
...
[^21]: The data skills gap is relevant to professional bodies, such as the Chartered Institute of Ecology and Environmental Management (CIEEM) a leading institute for professional ecologists, with a competency framework members are required to fulfill; this does not mention statistics or data science.
[^22]: Excel will try converting a data entry into a date even if it is not.
[^23]: For example literate programming eliminates the countless copy and paste of a traditional reporting workflow.
[^24]: Released on the World Service _More or Less_ on 11 Feb 2023 and available for over a year.
