...
## Making Bat Data _Tidy_

Results of bat sound analysis can be untidy:

* two or more species in one cell (see Table @tbl-untidy1);
* count of bats (Tables @tbl-untidy2); and, 
* two of more columns with species of same date and time.
* Longitude and Latitude columns with missing values

While the bat survey results shown in Table @tbl-tidy is an example of a _tidy_ data set; the data sets in Tables \@ref(tab:table02) and @tbl-untidy1 are _untidy_ and would need to be made tidy to undertake analysis.

Data preparation is not just a first step but must be repeated many times over during analysis; as new problems come to light, or new data is collected. Apart from making bat data into a _tidy format_, there are many other tasks involved in cleaning data: parsing dates and numbers, identifying missing values, correcting character encodings, matching similar but not identical values (such as those created by typos).
...
Table <- tibble(DateTime, Species)

knitr::kable(Table)

```

```{r tab.cap="Untidy Bat Data a Column of Counting the Bats", tab.id="table03"}
#| label: tbl-untidy2
...
Table <- tibble(DateTime, Species, Number)
