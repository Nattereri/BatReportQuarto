...
A full bat survey report produced, ***with one mouse click***, from sound analysis data (i.e after identifying the bat to species or genus with sound analysis). _One click reporting_ is achieved through literate programming [@knuth84]; a procedure of mixing code and descriptive writing, in order to undertake and explain data analysis simultaneously in the same document.  

The first few rows of bat survey data, post sound analysis, is shown in @tbl-surveydata; this is from of the `statics` data in the `iBats` package, is _tidy_ and has the minimum columns for undertaking effective analysis:
